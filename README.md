...
- **Audio**: `cpal` (background `.opus` track + a synthed exponentially-decaying pling on sprite overlap)
...
1. `./package-itch.sh` — runs `./build.sh`, then writes `pkg/rustwebtest-itch-YYYYMMDDTHHMMSSZ.zip` (UTC, ISO-8601 basic time; unique per second). Uses `7z` (`7z a -tzip`). Requires [7-Zip](https://www.7-zip.org/) on `PATH`. Override path with `ZIP_NAME=…`.
...
| [src/audio.rs](src/audio.rs) | cpal BGM playback + synth `Pling` `Source` |
